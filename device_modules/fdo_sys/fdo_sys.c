	    ...
	    bool *has_more, bool *is_more, size_t mtu)
{
	...
	case FDO_SI_FAILURE:
		...
		if (!process_data(FDO_SYS_MOD_MSG_EXIT, NULL, 0, NULL, NULL,
				  NULL, NULL, NULL)) {
	...
	case FDO_SI_GET_DSI_COUNT:
		// return the total number of messages that will be sent in THIS
		// message alone we are always sending 1 message at all times.
		// However, this flag 'case' won't be encountered since this is
		// not invoked by the 'lib/' as of now.
	...
	case FDO_SI_GET_DSI:
		...
		if (mtu == 0 || !module_message || !module_val ||
		    !module_val_sz) {
			result = FDO_SI_CONTENT_ERROR;
			goto end;
		}
		...
		if (write_type == FDO_SYS_MOD_MSG_STATUS_CB) {
			...
			if (file_sz == 0 || file_sz != get_file_sz(filename) ||
				...
				bin_len =
				    file_remaining > mtu ? mtu : file_remaining;
				bin_data =
				    ModuleAlloc(bin_len * sizeof(uint8_t));
				if (!bin_data) {
					LOG(LOG_DEBUG,
	...
	case FDO_SI_SET_OSI:
		...
		strcmp_s(module_message, FDO_MODULE_MSG_LEN, "filedesc",
			 &strcmp_filedesc);
		strcmp_s(module_message, FDO_MODULE_MSG_LEN, "write",
			 &strcmp_write);
		strcmp_s(module_message, FDO_MODULE_MSG_LEN, "exec",
			 &strcmp_exec);
		strcmp_s(module_message, FDO_MODULE_MSG_LEN, "exec_cb",
			 &strcmp_execcb);
		strcmp_s(module_message, FDO_MODULE_MSG_LEN, "status_cb",
			 &strcmp_statuscb);
		strcmp_s(module_message, FDO_MODULE_MSG_LEN, "fetch",
			 &strcmp_fetch);
		...
		if (strcmp_filedesc != 0 && strcmp_exec != 0 &&
			...
			LOG(LOG_DEBUG,
			    "Module fdo_sys - Invalid moduleMessage\n");
		...
		if (!fdor_parser_init(fdor)) {
			LOG(LOG_DEBUG,
			    "Module fdo_sys - Failed to init FDOR parser\n");
			goto end;
		}
		...
		if (strcmp_filedesc == 0) {
			...
			if (bin_len == 0) {
				LOG(LOG_DEBUG,
				    "Module fdo_sys - Empty value received for "
				    "fdo_sys:filedesc\n");
			...
			if (!fdor_text_string(fdor, (char *)bin_data,
					      bin_len)) {
				LOG(LOG_DEBUG, "Module fdo_sys - Failed to "
			...
			if (!fdor_byte_string(fdor, bin_data, bin_len)) {
				LOG(LOG_DEBUG,
				    "Module fdo_sys - Failed to read value for "
				    "fdo_sys:write\n");
				goto end;
			}

			if (!process_data(FDO_SYS_MOD_MSG_WRITE, bin_data,
					  bin_len, filename, NULL, NULL, NULL,
					  NULL)) {
				LOG(LOG_DEBUG,
				    "Module fdo_sys - Failed to process value "
				    "for fdo_sys:write\n");
				goto end;
			}
			result = FDO_SI_SUCCESS;
			...
			if (exec_array_length == 0) {
				LOG(LOG_DEBUG,
				    "Module fdo_sys - Empty array received for "
			...
			if (!fdor_start_array(fdor)) {
				LOG(LOG_DEBUG,
				    "Module fdo_sys - Failed to start "
				    "fdo_sys:exec/exec_cb array\n");
				goto end;
			}

			// allocate memory for exec_instr
			exec_instr = (char **)ModuleAlloc(
			    sizeof(char *) * (exec_array_length + 1));
			if (!exec_instr) {
				LOG(LOG_DEBUG,
				    "Module fdo_sys - Failed to alloc for "
				    "fdo_sys:exec/exec_cb instructions\n");
				goto end;
			}

			for (exec_array_index = 0;
			     exec_array_index < exec_array_length;
			     exec_array_index++) {
				exec_instr[exec_array_index] =
				    (char *)ModuleAlloc(sizeof(char) *
							MOD_MAX_EXEC_ARG_LEN);
				...
				if (!fdor_string_length(
					fdor, &exec_instructions_sz) ||
				...
				if (!fdor_text_string(
					fdor, exec_instr[exec_array_index],
					exec_instructions_sz)) {
					LOG(LOG_DEBUG,
					    "Module fdo_sys - Failed to read "
					    "fdo_sys:exec/exec_cb text\n");
				...
				if (exec_array_index == 1) {
					if (memset_s(filename, sizeof(filename),
						     0) != 0) {
						LOG(LOG_DEBUG,
						    "Module fdo_sys - Failed "
			...
			if (strcmp_exec == 0) {
				if (!process_data(FDO_SYS_MOD_MSG_EXEC, NULL, 0,
						  filename, exec_instr, NULL,
						  NULL, NULL)) {
					LOG(LOG_DEBUG,
					    "Module fdo_sys - Failed to "
				...
				if (!process_data(FDO_SYS_MOD_MSG_EXEC_CB, NULL,
						  0, filename, exec_instr,
			...
			if (!fdor_array_length(fdor, &status_cb_array_length)) {
				LOG(LOG_DEBUG,
				    "Module fdo_sys - Failed to process "
				    "fdo_sys:status_cb array length\n");
				goto end;
			}
			if (status_cb_array_length != 3) {
				LOG(LOG_DEBUG,
				    "Module fdo_sys - Invalid number of items "
			...
			if (!process_data(
				FDO_SYS_MOD_MSG_STATUS_CB, bin_data, bin_len,
				NULL, NULL, &status_cb_iscomplete,
