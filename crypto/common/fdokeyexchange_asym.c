 ...
 * protocol spec. These APIs inturn call crypto abstraction layer of FDO.
 */
...
#define DEVICE_RANDOM_SIZE FDO_ASYM_DEV_RANDOM
#else
/* ASYM3072 Generate Device Random bits(768) */
#define DEVICE_RANDOM_SIZE FDO_ASYM3072_DEV_RANDOM
...
typedef struct {
	...
	fdo_public_key_t *_encrypt_key; // key used to encrypt my secret B
} rsa_context_t;
...
int32_t crypto_hal_kex_init(void **context)
{
	...
	key_ex_data = fdo_alloc(sizeof(rsa_context_t));
	if (!key_ex_data) {
		return ret;
	}
	...
	if (fdo_crypto_random_bytes(key_ex_data->_Device_random,
...
static bool compute_publicBAsym(rsa_context_t *key_ex_data)
{
	...
	if (key_ex_data->_encrypt_key != NULL) {
		...
		fdo_public_key_to_string(key_ex_data->_encrypt_key,
					 debug_buffer, 2048);
		...
		key_ex_data->_publicB_length = crypto_hal_rsa_encrypt(
		     ...
		     * too from key_exdata
		     */
		    FDO_PK_HASH_SHA256, FDO_CRYPTO_PUB_KEY_ENCODING_RSA_MOD_EXP,
		    FDO_CRYPTO_PUB_KEY_ALGO_RSA, key_ex_data->_Device_random,
		...
		key_ex_data->_publicB =
			fdo_alloc(key_ex_data->_publicB_length);
		...
		if (key_ex_data->_Device_random) {
			...
			fdo_byte_array_to_string(pB, debug_buffer, 2048);
			if (debug_buffer[0] != 0)
				LOG(LOG_DEBUG, "rsa_encrypt result : %s.\n",
				    debug_buffer);
			fdo_byte_array_free(pB);
		}
#endif
 ...
 * @return none
 */
int32_t set_encrypt_key_asym(void *data, fdo_public_key_t *encrypt_key)
{
				   ...
				   uint32_t peer_rand_length)
{
	...
	shse = fdo_alloc(allocated_shse_size);
	if (!shse) {
		LOG(LOG_ERROR, "Memalloc failed\n");
		goto exit;
	}
