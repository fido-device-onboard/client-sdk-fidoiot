...
int32_t crypto_hal_random_bytes(uint8_t *random_buffer, size_t num_bytes)
{
	...
	if (NULL == random_buffer) {
		return -1;
	} else if (TEE_SUCCESS != fdo_heci_generate_random(
				      &fdo_cse_handle, random_buffer,
				      (uint32_t)num_bytes, &fdo_status) ||
			    ...
			    uint8_t *output, size_t output_length)
{
	...
	if (TEE_SUCCESS != fdo_heci_update_file(&fdo_cse_handle, OVH_FILE_ID,
						buffer, buffer_length, output,
						output_length, &fdo_status) ||
	    FDO_STATUS_SUCCESS != fdo_status) {
				  ...
				  size_t *maroe_length)
{
	...
	if (TEE_SUCCESS != fdo_heci_ecdsa_device_sign_challenge(
			       &fdo_cse_handle, (uint8_t *)data,
			       (uint32_t)data_len, message_signature,
			       message_sig_len, eat_maroe, &mp_len,
			       &fdo_status) ||
	    FDO_STATUS_SUCCESS != fdo_status) {
