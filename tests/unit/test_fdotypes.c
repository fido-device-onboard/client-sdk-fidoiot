...
#endif
{
	...
	uint8_t hmac_cbor[] = {
	    0x82, 0x06, 0x58, 0x30, 0x89, 0x5B, 0xD7, 0x23, 0x65, 0xFE, 0xE9,
	    0x3F, 0x89, 0x65, 0xBB, 0x5E, 0xB7, 0xDF, 0x6E, 0x74, 0xF6, 0xA8,
	    0x64, 0x21, 0xA7, 0x22, 0x74, 0xC5, 0xAC, 0xC5, 0x48, 0x81, 0x3E,
	    0x8F, 0x60, 0x1A, 0x05, 0xE4, 0xA6, 0x28, 0xDC, 0x79, 0x1E, 0x30,
	    0xCB, 0x49, 0x6E, 0x69, 0xB9, 0x9B, 0x0F, 0x1C};
	...
	memcpy_s(fdor->b.block, sizeof(hmac_cbor), hmac_cbor,
		 sizeof(hmac_cbor));
...
#endif
{
	...
	uint8_t ipv4_cbor[] = {0x44, 0x7F, 0x00, 0x00, 0x01};

	fdor = fdo_alloc(sizeof(fdor_t));
	TEST_ASSERT_NOT_NULL(fdor);
	TEST_ASSERT_TRUE(fdor_init(fdor));
	TEST_ASSERT_TRUE(fdo_block_alloc(&fdor->b));
	memcpy_s(fdor->b.block, sizeof(ipv4_cbor), ipv4_cbor,
		 sizeof(ipv4_cbor));
...
#endif
{
	...
	uint8_t pubkey_cbor[] = {
	    0x83, 0x0A, 0x01, 0x58, 0x60, 0x92, 0x11, 0x12, 0xFD, 0x17, 0xEC,
	    0x7F, 0x33, 0x05, 0x24, 0xFD, 0x4D, 0xE3, 0x18, 0xE5, 0x0A, 0x85,
	    0x93, 0x3A, 0xDA, 0xFF, 0x6B, 0x2F, 0x1B, 0x7C, 0x51, 0xE5, 0x5D,
	    0xFB, 0x52, 0x71, 0x02, 0x33, 0x94, 0xAE, 0x3F, 0x7D, 0x1F, 0xDE,
	    0x29, 0x82, 0x27, 0x30, 0x4A, 0x01, 0xE5, 0x4B, 0x08, 0x90, 0xFE,
	    0x98, 0xA3, 0xEA, 0x09, 0xD4, 0x01, 0x1C, 0xE0, 0xCC, 0xC5, 0x37,
	    0xCD, 0xCD, 0xFF, 0x55, 0x3B, 0x21, 0x83, 0x24, 0x93, 0x3C, 0x72,
	    0x55, 0xE2, 0x49, 0xB4, 0xA3, 0xF5, 0x38, 0x0E, 0x0D, 0x16, 0x58,
	    0x97, 0x15, 0xCE, 0x9F, 0x0B, 0xC7, 0xB2, 0xE8, 0x0F, 0xAF, 0xB6,
	    0x15, 0x89};
	...
	TEST_ASSERT_TRUE(fdor_init(fdor));
	TEST_ASSERT_TRUE(fdo_block_alloc(&fdor->b));
	memcpy_s(fdor->b.block, sizeof(pubkey_cbor), pubkey_cbor,
		 sizeof(pubkey_cbor));
...
#endif
{
	...
	uint8_t rv_cbor[] = {0x81, 0x85, 0x82, 0x05, 0x69, 0x6C, 0x6F,
			     0x63, 0x61, 0x6C, 0x68, 0x6F, 0x73, 0x74,
			     0x82, 0x03, 0x19, 0x1F, 0x68, 0x82, 0x0C,
			     0x01, 0x82, 0x02, 0x44, 0x7F, 0x00, 0x00,
...
#endif
{
	// TO-DO: Update sample CBOR encoded Encrypted Message body with COSE
	// Tag
	TEST_IGNORE();
	fdor_t *fdor = NULL;
	...
	uint8_t enc_msg_cbor[] = {
	    0x83, 0x44, 0xa1, 0x01, 0x18, 0x20, 0xa1, 0x05, 0x47, 0xfe,
	    0x6b, 0x0d, 0x51, 0x5a, 0x74, 0xe6, 0x54, 0x48, 0x66, 0x0b,
	    0x35, 0xbc, 0x04, 0xd1, 0x05, 0x07, 0x9a, 0x0a, 0x2f, 0xfa,
	    0x25, 0x28, 0xd3, 0x53, 0x5e, 0xb5, 0x1e};
#else
	uint8_t enc_msg_cbor[] = {
	    0x83, 0x44, 0xa1, 0x01, 0x18, 0x21, 0xa1, 0x05, 0x47, 0xfe,
	    0x6b, 0x0d, 0x51, 0x5a, 0x74, 0xe6, 0x54, 0x48, 0x66, 0x0b,
	    0x35, 0xbc, 0x04, 0xd1, 0x05, 0x07, 0x9a, 0x0a, 0x2f, 0xfa,
	    0x25, 0x28, 0xd3, 0x53, 0x5e, 0xb5, 0x1e};
#endif
	...
	TEST_ASSERT_TRUE(fdor_init(fdor));
	TEST_ASSERT_TRUE(fdo_block_alloc(&fdor->b));
	memcpy_s(fdor->b.block, sizeof(enc_msg_cbor), enc_msg_cbor,
		 sizeof(enc_msg_cbor));
...
#endif
{
	// test this when encryption is mocked, add
	// 'fdo_encrypted_packet_unwind()' as well ignore the test for now
	TEST_IGNORE();
...
#endif
{
	...
	bool ret;
	uint8_t cose_cbor[] = {0x84, 0x44, 0xA1, 0x01, 0x38, 0x22,
			       0xA0, 0x41, 0x02, 0x41, 0x02};

	fdor = fdo_alloc(sizeof(fdor_t));
	TEST_ASSERT_NOT_NULL(fdor);
	TEST_ASSERT_TRUE(fdor_init(fdor));
	TEST_ASSERT_TRUE(fdo_block_alloc(&fdor->b));
	memcpy_s(fdor->b.block, sizeof(cose_cbor), cose_cbor,
		 sizeof(cose_cbor));
...
#endif
{
	...
	uint8_t siginfo_cbor[] = {0x82, 0x38, 0x22, 0x40};
#else
	uint8_t siginfo_cbor[] = {0x82, 0x26, 0x40};
#endif
	...
	TEST_ASSERT_TRUE(fdor_init(fdor));
	TEST_ASSERT_TRUE(fdo_block_alloc(&fdor->b));
	memcpy_s(fdor->b.block, sizeof(siginfo_cbor), siginfo_cbor,
		 sizeof(siginfo_cbor));
...
#endif
{
	...
	// update existing key with updated value is a positive case
	ret = fdo_service_info_add_kv_bool(si, "dummy_key", false);
	TEST_ASSERT_TRUE(ret);
...
#endif
{
	...
	// update existing key with updated value is a positive case
	ret = fdo_service_info_add_kv_int(si, "dummy_key", 7);
	TEST_ASSERT_TRUE(ret);
...
#endif
{
	...
	ret = fdo_serviceinfo_invalid_modname_add(
	    "testmod1", &serviceinfo_invalid_modnames);
	TEST_ASSERT_TRUE(ret);
	...
	ret = fdo_serviceinfo_invalid_modname_add(
	    "testmod2", &serviceinfo_invalid_modnames);
	TEST_ASSERT_TRUE(ret);
