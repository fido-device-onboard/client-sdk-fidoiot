...
void test_encode_decode(void)
{
	...
	fdo_byte_array_t *mstring =
	    fdo_byte_array_alloc(sizeof(fdo_byte_array_t));
	TEST_ASSERT_NOT_NULL(mstring);
	if (memset_s(mstring->bytes, mstring->byte_sz * sizeof(uint8_t), 0) !=
	    0) {
	...
	for (i = 0; i < finalLength; i++) {
		LOG(LOG_DEBUGNTS, "%02x", fdow->b.block[i]);
	}
	LOG(LOG_INFO, "\nEncoding finished successfully\n");
	fdow->b.block_size = finalLength;

	fdor->b.block_size = finalLength;
	memcpy_s(fdor->b.block, CBOR_BUFFER_LENGTH, fdow->b.block,
		 fdow->b.block_size);
