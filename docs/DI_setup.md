...
> **Note:** To run  SDO Client-SDK binaries from the release package, the following configuration can be used when launching  SDO CRIs (required before running the DI protocol):
>
...
>
> All the required certs and cert-chains are present under the `<path-to-sdo-client-sdk>/data` directory.

Detailed steps and configuration needed to start SDO manufacturer can be found in the `<release-package-dir>/cri/README.md` document.
...
> **Note:** If you are not running the default SDO Client-SDK binaries from the release-package and have your own Intel® Enhanced Privacy ID (Intel® EPID) key or EC keys and certs, do the following:
>   - If device attestation method is Intel EPID, place the Intel EPID-private-key in the `data/` directory and name it `epidprivkey.dat`.
>
>   - If device attestation method is Elliptic Curve Digital Signature Algorithm (ECDSA), place the ECDSA-private-key in the `data/` directory and name it `ecdsaXXXprivkey.dat`(`ecdsa256privkey.dat` for curve type `P-256` and `ecdsa384privkey.dat` for curve type `P-384`). For Privacy-Enhanced Mail (PEM)-formatted private key, use `ecdsaXXXprivkey.pem` in a similar way (`ecdsa256privkey.pem` or `ecdsa384privkey.pem`).
