...
int32_t msg63(fdo_prot_t *ps)
{
	...
	// OVEntryNum value must match with the requested Ownership Voucher
	// index
	...
	if (!fdo_cose_write_sigstructure(cose->cose_ph, cose->cose_payload,
	    ...
	    !cose_sig_structure) {
		LOG(LOG_ERROR,
		    "TO2.OVNextEntry: Failed to write COSE Sig_structure\n");
		goto err;
	}
	...
	if (!fdo_signature_verification(cose_sig_structure,
		...
		LOG(LOG_ERROR,
		    "TO2.OVNextEntry: Failed to verify OVEntry signature\n");
		goto err;
	}
	LOG(LOG_DEBUG,
	    "TO2.OVNextEntry: OVEntry Signature verification successful\n");

	// Generate COSE as CBOR bytes again that is used to calculate
	// OVEHashPrevEntry.
	if (!fdo_cose_write(&ps->fdow, cose)) {
		LOG(LOG_ERROR, "TO2.OVNextEntry: Failed to write COSE for "
			       "OVEHashPrevEntry\n");
		goto err;
	}
	// Get encoded COSE and copy
	if (!fdow_encoded_length(&ps->fdow, &ps->fdow.b.block_size)) {
		LOG(LOG_ERROR, "TO2.OVNextEntry: Failed to get encoded COSE "
			       "length for OVEHashPrevEntry\n");
		goto err;
	}
	cose_encoded = fdo_byte_array_alloc(ps->fdow.b.block_size);
	if (!cose_encoded) {
		LOG(LOG_ERROR, "TO2.OVNextEntry: Failed to alloc encoded COSE "
			       "for OVEHashPrevEntry\n");
		goto err;
	}
	if (0 != memcpy_s(cose_encoded->bytes, cose_encoded->byte_sz,
			  ps->fdow.b.block, ps->fdow.b.block_size)) {
		LOG(LOG_ERROR, "TO2.OVNextEntry: Failed to copy encoded COSE "
			       "for OVEHashPrevEntry\n");
		goto err;
	}
	...
	if (0 != memcpy_s(ps->fdor.b.block, ps->fdor.b.block_size,
		...
		LOG(LOG_ERROR, "TO2.OVNextEntry: Failed to copy encoded "
			       "COSEPayload for OVEHashPrevEntry\n");
		goto err;
	}
	...
	if (!fdor_parser_init(&ps->fdor)) {
		LOG(LOG_ERROR,
		    "TO2.OVNextEntry: Failed to initilize FDOR parser\n");
		goto err;
	}
	...
	if (!fdor_array_length(&ps->fdor, &num_payloadbasemap_items) ||
		...
		LOG(LOG_ERROR,
		    "TO2.OVNextEntry: Failed to read array length\n");
		goto err;
	}

	if (!fdor_start_array(&ps->fdor)) {
		LOG(LOG_ERROR,
		    "TO2.OVNextEntry: Failed to start OVEntryPayload array\n");
		goto err;
	}
	...
	if (!temp_hash_hp || fdo_hash_read(&ps->fdor, temp_hash_hp) <= 0) {
		LOG(LOG_ERROR, "TO2.OVNextEntry: Failed to read "
			       "OVEntryPayload.OVEHashPrevEntry\n");
		fdo_hash_free(temp_hash_hp);
		goto err;
	}

	if (temp_hash_hp->hash_type != FDO_CRYPTO_HASH_TYPE_USED) {
		LOG(LOG_ERROR, "TO2.OVNextEntry: Invalid Hash Type at "
			       "OVEntryPayload.OVEHashPrevEntry\n");
		fdo_hash_free(temp_hash_hp);
		goto err;
	}
	...
	if (fdor_is_value_null(&ps->fdor)) {
		...
		if (!fdor_string_length(&ps->fdor, &ove_extra_len) ||
		    ove_extra_len == 0) {
			LOG(LOG_DEBUG, "TO2.OVNextEntry: Unable to decode "
				       "length of OVEExtra!\n");
			goto err;
		}
		temp_ove_extra = fdo_byte_array_alloc(ove_extra_len);
		if (!temp_ove_extra) {
			LOG(LOG_ERROR, "TO2.OVNextEntry: Failed to alloc for "
				       "OVEExtra as bstr\n");
			goto err;
		}
		if (!fdor_byte_string(&ps->fdor, temp_ove_extra->bytes,
				      temp_ove_extra->byte_sz)) {
			LOG(LOG_ERROR, "TO2.OVNextEntry: Failed to read "
				       "OVEExtra as bstr\n");
			goto err;
		}
	...
	if (!temp_pk) {
		LOG(LOG_ERROR, "TO2.OVNextEntry: Failed to read "
			       "OVEntryPayload.OVEPubKey\n");
		goto err;
	}

	if (!fdor_end_array(&ps->fdor)) {
		LOG(LOG_ERROR,
		    "TO2.OVNextEntry: Failed to end OVEntryPayload array\n");
		goto err;
	}
	...
	// Compare OVEHashPrevEntry (msg61 data) with the OVEHashPrevEntry from
	// this message
	if (memcmp_s(ps->ovoucher->ov_entries->hp_hash->hash->bytes,
	    ...
	    result_memcmp) {
		LOG(LOG_ERROR,
		    "TO2.OVNextEntry: Failed to match OVEHashPrevEntry %d\n",
		    ps->ov_entry_num);
		goto err;
	}

	// Compare OVEHashHdrInfo (msg61 data) with the OVEHashHdrInfo from this
	// message
	if (memcmp_s(ps->ovoucher->ov_entries->hc_hash->hash->bytes,
		     ...
		     temp_entry->hc_hash->hash->byte_sz, &result_memcmp) ||
	    result_memcmp) {
		LOG(LOG_ERROR,
		    "TO2.OVNextEntry: Failed to match OVEHashHdrInfo %d\n",
		    ps->ov_entry_num);
		goto err;
	}
	...
	if (!current_hp_hash) {
		LOG(LOG_ERROR,
		    "TO2.OVNextEntry: Failed to alloc current OVEntry hash!\n");
		goto err;
	}
	...
	if (0 != fdo_crypto_hash(cose_encoded->bytes, cose_encoded->byte_sz,
		...
		LOG(LOG_ERROR, "TO2.OVNextEntry: Failed to generate current "
			       "OVEntry hash!\n");
		goto err;
	}
	...
	if (ps->ov_entry_num < ps->ovoucher->num_ov_entries) {
		...
		if (!fdow_encoder_init(&ps->fdow)) {
			LOG(LOG_ERROR, "TO2.OVNextEntry: Failed to initialize "
				       "FDOW encoder\n");
			goto err;
		}
	} else {
		...
		if (!fdo_compare_public_keys(ps->owner_public_key,
					     temp_entry->pk)) {
			LOG(LOG_ERROR, "TO2.OVNextEntry: Failed to match "
				       "Owner's pk to OVHdr pk!\n");
			goto err;
		}
