...
int32_t msg67(fdo_prot_t *ps)
{
	...
	if (fdor_is_value_signed_int(&ps->fdor)) {
		if (!fdor_unsigned_int(&ps->fdor, &rec_maxDeviceServiceInfoSz)) {
			LOG(LOG_ERROR,
	...
	LOG(LOG_DEBUG, "TO2.OwnerServiceInfoReady: Received maxDeviceServiceInfoSz = %llu\n", rec_maxDeviceServiceInfoSz);
	if (rec_maxDeviceServiceInfoSz < MIN_SERVICEINFO_SZ) {
		...
		LOG(LOG_DEBUG,"TO2.OwnerServiceInfoReady: Received maxDeviceServiceInfoSz is less than the minimum size supported. Defaulting to %llu\n", ps->maxDeviceServiceInfoSz);
	} else if (rec_maxDeviceServiceInfoSz > ps->maxDeviceServiceInfoSz) {
		// nothing to do, just log it
		LOG(LOG_DEBUG,"TO2.OwnerServiceInfoReady: Received maxDeviceServiceInfoSz is more than the maximum size supported. Defaulting to %llu\n", ps->maxDeviceServiceInfoSz);
	} else {
	...
	LOG(LOG_DEBUG, "TO2.OwnerServiceInfoReady: Expected Maximum Device ServiceInfo size is %llu\n", ps->maxDeviceServiceInfoSz);
