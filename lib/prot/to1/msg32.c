...
int32_t msg32(fdo_prot_t *ps)
{
	...
	if (!fdo_eat_write_payloadbasemap(&ps->fdow, &payloadbasemap)) {
		LOG(LOG_ERROR,
		    "TO1.ProveToRV: Failed to write EATPayloadBaseMap\n");
		goto err;
	}
	...
	if (!fdow_encoded_length(&ps->fdow, &payload_length) ||
	    payload_length == 0) {
		LOG(LOG_ERROR,
		    "TO1.ProveToRV: Failed to read EATPayload length\n");
		goto err;
	}
	...
	if (!encoded_payloadbasemap) {
		LOG(LOG_ERROR,
		    "TO1.ProveToRV: Failed to alloc for encoded EATPayload\n");
		goto err;
	}
	...
	if (!fdow_encoder_init(&ps->fdow)) {
		LOG(LOG_ERROR,
		    "TO1.ProveToRV: Failed to initilize FDOW encoder\n");
		goto err;
	}
	...
	if (!fdo_eat_write_sigstructure(eat->eat_ph, eat->eat_payload, NULL,
	    ...
	    !eat_sig_structure) {
		LOG(LOG_ERROR,
		    "TO1.ProveToRV: Failed to write COSE Sig_structure\n");
		goto err;
	}
