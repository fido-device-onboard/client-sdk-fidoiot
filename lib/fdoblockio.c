...
/**
 * Clear the internal buffer of the given fdo_block_t struct by setting the
 * contents to 0, upto the internal block size.
 *
...
void fdo_block_reset(fdo_block_t *fdob)
{
	...
	if (fdob->block) {
		if (fdob->block_size &&
		    memset_s(fdob->block, fdob->block_size, 0)) {
...
/**
 * Allocate memory for the underlying block with default size of
 * 'CBOR_BUFFER_LENGTH'.
 *
 * @param fdo_block_t - struct containg the buffer and its size
...
/**
 * Clear the contents of the given fdow_t struct alongwith its internal
 * fdo_block_t buffer. Memory must have been previously allocated for both
 * fdow_t struct and its internal fdo_block_t.
 *
...
/**
 * Set the FDO Type for the given fdow_t struct to prepare for the next
...
/**
 * Allocates for the internal fdow_cbor_encoder_t struct to initialize
 * TinyCBOR's CborEncoder that actually does the CBOR encoding. The newly
 * initialized CborEncoder is provided with the buffer that will be used to
 * store the CBOR-encoded data, and its maximum size. It is the root encoder
 * onto which other CBOR encoders can be added. The next and previous pointers
 * to NULL. After this, the given fdow_t struct is ready to do CBOR encoding.
 *
 * @param fdow_t - struct fdow_t
 * @return true if the operation was a success, false otherwise
 */
bool fdow_encoder_init(fdow_t *fdow)
{
	...
	cbor_encoder_init(&fdow->current->cbor_encoder, fdow->b.block,
			  fdow->b.block_size, 0);
	return true;
}
 ...
 * writes the tag into the pre-initialized buffer. At the end of this, every
 * write operation would be done using the newly created CborEncoder making them
 * the items of this array, until all the items are written. The array needs to
 * be closed using the method fdow_end_array().
 *
 * @param fdow_t - struct fdow_t
...
bool fdow_start_array(fdow_t *fdow, size_t array_items)
{
	...
	if (cbor_encoder_create_array(&fdow->current->previous->cbor_encoder,
				      &fdow->current->cbor_encoder,
				      array_items) != CborNoError) {
		LOG(LOG_ERROR,
		    "CBOR encoder: Failed to start Major Type 4 (array)\n");
		return false;
	}
	return true;
}
 ...
 * writes the tag into the pre-initialized buffer. At the end of this, every
 * write operation would be done using the newly created CborEncoder making them
 * the key-value pairs of this map, until all the items are written. The map
 * needs to be closed using the method fdow_end_map().
 *
 * @param fdow_t - struct fdow_t
...
bool fdow_start_map(fdow_t *fdow, size_t map_items)
{
	...
	if (cbor_encoder_create_map(&fdow->current->previous->cbor_encoder,
				    &fdow->current->cbor_encoder,
				    map_items) != CborNoError) {
		LOG(LOG_ERROR,
		    "CBOR encoder: Failed to start Major Type 5 (map)\n");
		return false;
	}
	return true;
}
...
bool fdow_byte_string(fdow_t *fdow, uint8_t *bytes, size_t byte_sz)
{
	...
	if (cbor_encode_byte_string(&fdow->current->cbor_encoder, bytes,
				    byte_sz) != CborNoError) {
		LOG(LOG_ERROR,
		    "CBOR encoder: Failed to write Major Type 2 (bstr)\n");
		return false;
	}
	return true;
}
...
bool fdow_text_string(fdow_t *fdow, char *bytes, size_t byte_sz)
{
	...
	if (cbor_encode_text_string(&fdow->current->cbor_encoder, bytes,
				    byte_sz) != CborNoError) {
		LOG(LOG_ERROR,
		    "CBOR encoder: Failed to write Major Type 3 (tstr)\n");
		return false;
	}
	return true;
}
...
bool fdow_signed_int(fdow_t *fdow, int value)
{
	...
	if (cbor_encode_int(&fdow->current->cbor_encoder, value) !=
	    CborNoError) {
		LOG(LOG_ERROR, "CBOR encoder: Failed to write Major Type 1 "
			       "(negative int)\n");
		return false;
	}
	return true;
...
bool fdow_unsigned_int(fdow_t *fdow, uint64_t value)
{
	...
	if (cbor_encode_uint(&fdow->current->cbor_encoder, value) !=
	    CborNoError) {
		LOG(LOG_ERROR,
		    "CBOR encoder: Failed to write Major Type 0 (uint)\n");
		return false;
	}
	return true;
}
...
bool fdow_boolean(fdow_t *fdow, bool value)
{
	...
	if (cbor_encode_boolean(&fdow->current->cbor_encoder, value) !=
	    CborNoError) {
		LOG(LOG_ERROR,
		    "CBOR encoder: Failed to write Major Type 7 (bool)\n");
		return false;
	}
	return true;
}
...
bool fdow_null(fdow_t *fdow)
{
	...
	if (cbor_encode_null(&fdow->current->cbor_encoder) != CborNoError) {
		LOG(LOG_ERROR,
		    "CBOR encoder: Failed to write Major Type 7 (NULL)\n");
		return false;
	}
	return true;
}
 ...
 *
 * It moves back to previous CborEncoder and frees the node containing the
 * current CborEncoder (next), closing the array. At the end of this, every
 * write operation would be done using the previous CborEncoder (represented by
 * current).
 *
 * @param fdow_t - struct fdow_t
 * @return true if the operation was a success, false otherwise
...
bool fdow_end_array(fdow_t *fdow)
{
	...
	if (cbor_encoder_close_container_checked(
		...
		LOG(LOG_ERROR,
		    "CBOR encoder: Failed to end Major Type 4 (array)\n");
		return false;
	}
 ...
 *
 * It moves back to previous CborEncoder and frees the node containing the
 * current CborEncoder (next), closing the map. At the end of this, every write
 * operation would be done using the previous CborEncoder (represented by
 * current).
 *
 * @param fdow_t - struct fdow_t
 * @return true if the operation was a success, false otherwise
...
bool fdow_end_map(fdow_t *fdow)
{
	...
	if (cbor_encoder_close_container_checked(
		...
		&fdow->current->cbor_encoder) != CborNoError) {
		LOG(LOG_ERROR,
		    "CBOR encoder: Failed to end Major Type 5 (map)\n");
		return false;
	}
	// move backwards and free next
	fdow->current = fdow->current->previous;
...
/**
 * Store the length of the CBOR data that has been written so far to the
 * supplied buffer (fdow_t.fdo_block_t.block) in the output size_t variable.
 *
 * @param fdow_t - struct fdow_t
...
bool fdow_encoded_length(fdow_t *fdow, size_t *length)
{
	...
	*length = cbor_encoder_get_buffer_size(&fdow->current->cbor_encoder,
					       fdow->b.block);
	return true;
}

/**
 * Clear and deallocate the internal buffer (fdow_t.fdo_block_t.block) alongwith
 * the current node.
 *
 * @param fdow_t - struct fdow_t
 */
...
/**
 * Clear the contents of the given fdor_t struct alongwith its internal
 * fdo_block_t buffer. Memory must have been previously allocated for both
 * fdor_t struct and its internal fdo_block_t.
 *
...
/**
 * Allocates for the internal fdor_cbor_decoder_t struct to initialize
 * TinyCBOR's CborParser that actually does the CBOR decoding. The newly
 * initialized CborDecoder is provided with the buffer that contains the
 * CBOR-encoded data (the data to be decoded), its maximum size, and TinyCbor's
 * CborValue. It is the root decoder that takes as many CborValue's as the
 * number of arrays/maps to be read. The next and previous pointers to NULL.
 * After this, the given fdor_t struct is ready to do CBOR decoding.
 *
 * @param fdor_t - struct fdor_t
 * @return true if the operation was a success, false otherwise
 */
bool fdor_parser_init(fdor_t *fdor)
{
	...
	if (cbor_parser_init(fdor->b.block, fdor->b.block_size, 0,
			     &fdor->cbor_parser,
			     &fdor->current->cbor_value) != CborNoError) {
		LOG(LOG_ERROR,
		    "CBOR decoder: Failed to initialize CBOR Parser\n");
		return false;
	}
	return true;
}
 ...
 * would be done using the newly created CborValue treating them as the items of
 * this array. The array needs to be closed using the method fdor_end_array().
 *
 * @param fdor_t - struct fdor_t
 * @return true if the operation was a success, false otherwise
 */
bool fdor_start_array(fdor_t *fdor)
{
	...
	if (!cbor_value_is_array(&fdor->current->previous->cbor_value) ||
	    cbor_value_enter_container(&fdor->current->previous->cbor_value,
				       &fdor->current->cbor_value) !=
		CborNoError) {
		LOG(LOG_ERROR,
		    "CBOR decoder: Failed to start Major Type 4 (array)\n");
		return false;
	}
	return true;
}
 ...
 * reads the tag from the input buffer. At the end of this, every read operation
 * would be done using the newly created CborValue treating them as the items of
 * this map. The map needs to be closed using the method fdor_end_map().
 *
 * @param fdor_t - struct fdor_t
 * @return true if the operation was a success, false otherwise
 */
...
/**
 * Store the number of items in the CBOR array (Major Type 4) into the supplied
 * size_t variable.
 *
 * @param fdor_t - struct fdor_t
 * @param length - output variable where the array's number of items will be
 * stored
 * @return true if the operation was a success, false otherwise
 */
bool fdor_array_length(fdor_t *fdor, size_t *length)
{
	...
	if (!cbor_value_is_array(&fdor->current->cbor_value) ||
	    cbor_value_get_array_length(&fdor->current->cbor_value, length) !=
		CborNoError) {
		LOG(LOG_ERROR, "CBOR decoder: Failed to read length of Major "
			       "Type 4 (array)\n");
		return false;
	}
	return true;
...
/**
 * Store the number of items in the CBOR map (Major Type 5) into the supplied
 * size_t variable.
 *
 * @param fdor_t - struct fdor_t
 * @param length - output variable where the array's number of items will be
...
bool fdor_map_length(fdor_t *fdor, size_t *length)
{
	...
	if (!cbor_value_is_map(&fdor->current->cbor_value) ||
	    cbor_value_get_map_length(&fdor->current->cbor_value, length) !=
		CborNoError) {
		LOG(LOG_ERROR, "CBOR decoder: Failed to read length of Major "
			       "Type 5 (map)\n");
		return false;
...
/**
 * Store the CBOR bstr/tstr (Major Type 2/3) length into the supplied size_t
 * variable.
 *
 * @param fdor_t - struct fdor_t
...
bool fdor_byte_string(fdor_t *fdor, uint8_t *buffer, size_t buffer_length)
{
	...
	if (!cbor_value_is_byte_string(&fdor->current->cbor_value) ||
	    cbor_value_copy_byte_string(&fdor->current->cbor_value, buffer,
					&buffer_length, NULL) != CborNoError) {
		LOG(LOG_ERROR,
		    "CBOR decoder: Failed to read Major Type 2 (bstr)\n");
		return false;
	}
	if (!fdor_next(fdor)) {
		return false;
...
bool fdor_text_string(fdor_t *fdor, char *buffer, size_t buffer_length)
{
	...
	if (!cbor_value_is_text_string(&fdor->current->cbor_value) ||
	    cbor_value_copy_text_string(&fdor->current->cbor_value, buffer,
					&buffer_length, NULL) != CborNoError) {
		LOG(LOG_ERROR,
		    "CBOR decoder: Failed to read Major Type 3 (tstr)\n");
		return false;
	}
	if (!fdor_next(fdor)) {
		return false;
...
/**
 * Check if the current value is CBOR NULL (Major Type 7, Additional Info 22)
 * value.
 *
 * @param fdor_t - struct fdor_t
...
bool fdor_signed_int(fdor_t *fdor, int *result)
{
	...
	if (!cbor_value_is_integer(&fdor->current->cbor_value) ||
		...
		CborNoError) {
		LOG(LOG_ERROR, "CBOR decoder: Failed to read Major Type 1 "
			       "(negative int)\n");
		return false;
	}
	if (!fdor_next(fdor)) {
...
bool fdor_unsigned_int(fdor_t *fdor, uint64_t *result)
{
	...
	if (!cbor_value_is_unsigned_integer(&fdor->current->cbor_value) ||
		...
		CborNoError) {
		LOG(LOG_ERROR,
		    "CBOR decoder: Failed to read Major Type 0 (uint)\n");
		return false;
	}
	if (!fdor_next(fdor)) {
		return false;
...
bool fdor_boolean(fdor_t *fdor, bool *result)
{
	...
	if (!cbor_value_is_boolean(&fdor->current->cbor_value) ||
		...
		CborNoError) {
		LOG(LOG_ERROR,
		    "CBOR decoder: Failed to start Major Type 7 (bool)\n");
		return false;
	}
	if (!fdor_next(fdor)) {
		return false;
...
bool fdor_end_array(fdor_t *fdor)
{
	...
	if (!cbor_value_is_array(&fdor->current->previous->cbor_value) ||
		...
		CborNoError) {
		LOG(LOG_ERROR,
		    "CBOR decoder: Failed to end Major Type 4 (array)\n");
		return false;
	}
	// move backwards and free next
	fdor->current = fdor->current->previous;
...
/**
 * Determine if the given buffer points to a map and whether it contains
 * unread/unparsed elements (keys/values).
 *
 * @param fdor_t - struct fdor_t
 * @return true if the map contains keys/values to be read/parsed, false
 * otherwise
 */
...
bool fdor_is_valid_cbor(fdor_t *fdor)
{
	...
	if (cbor_value_validate(&fdor->current->cbor_value,
				CborValidateBasic) != CborNoError) {
		LOG(LOG_ERROR, "CBOR decoder: Incorrectly or non-CBOR encoded "
			       "stream encountered.\n");
		return false;
	}
	return true;
...
/**
 * Clear and deallocate the internal buffer (fdor_t.fdo_block_t.block) alongwith
 * the current node.
 *
 * @param fdor_t - struct fdor_t
 */
